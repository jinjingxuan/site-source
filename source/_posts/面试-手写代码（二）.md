...
> 解析url中参数为对象，考虑 key 重复，没有 val 和 转码
...
> 给一个有序数组，一个数，让输出数组中求和得到这个数的数字下标。
>
> 输入：[1,2,3,4,5]， 6 
...
> 开根号，要求位数 0.0001，考虑 0 - 1 开根号越来越大
...
```js
function requestLimit(urlList, limitNum) {

}
```

```js
// requestRetry('https://xxx', 3).then(data).catch(error)
