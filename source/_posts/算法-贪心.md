...
* 其中 sub[i] > 0  才能作为起始点
* 题目就变为找出sub数组中的索引值 i，从 i 开始依次相加判断剩余油量大于0直到循环一轮结束
* 时间复杂度 O(n^2)
...
* 同样的每个加油站的油量结余sub[i] = gas[i] - cost[i]
* 只需遍历一遍所有加油站，依次以每个加油站为始发站，累加路径油量结余和总结余
* 一旦路径油量结余小于零，说明[0, i]区间都不能作为起始位置，始发站改为i+1，再从0计算sub
* 如果最后总油量结余大于等于零那么一定可以跑完一圈，说明 各个站点的加油站 剩油量sub[i]相加一定是大于等于零的
* 时间复杂度 O(n)
